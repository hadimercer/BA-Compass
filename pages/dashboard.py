...
from components.db import run_query, set_last_active_project
        ...
        if create_project:
            if not project_name.strip():
                ...
                try:
                    ...
                    pid = str(inserted_rows[0]["project_id"])
                    st.session_state["active_project_id"] = pid
                    st.session_state["active_project_name"] = project_name.strip()
                    try:
                        set_last_active_project(current_user["user_id"], pid)
                    except RuntimeError:
                        pass  # Non-blocking — navigation must complete regardless
